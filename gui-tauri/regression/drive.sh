...
curl -s -o "$OUT/r1.body" -w "%{http_code}" -X POST "$B/v1/messages" \
  ...
  -d '{"model":"claude-3-opus-latest","max_tokens":5,"metadata":{"user_id":"slot0"},"messages":[{"role":"user","content":"hi"}]}' > "$OUT/r1.status"

# 3) 槽位0 的 [1m] 变体
curl -s -o /dev/null -X POST "$B/v1/messages" \
  -H "content-type: application/json" \
  -d '{"model":"claude-3-opus-latest[1m]","max_tokens":5,"metadata":{"user_id":"slot0-1m"},"messages":[{"role":"user","content":"hi"}]}'
...
curl -s -o /dev/null -X POST "$B/v1/messages" \
  -H "content-type: application/json" \
  -d '{"model":"claude-3-5-sonnet-latest","max_tokens":5,"output_config":{"effort":"low"},"metadata":{"user_id":"slot1-off-with-effort"},"messages":[{"role":"user","content":"hi"}]}'

# 5) 槽位2（te=high，请求不带 output_config → 兜底注入，与 v1 等价）
curl -s -o /dev/null -X POST "$B/v1/messages" \
  -H "content-type: application/json" \
  -d '{"model":"claude-3-sonnet-20240229","max_tokens":5,"metadata":{"user_id":"slot2-high"},"messages":[{"role":"user","content":"hi"}]}'
...
curl -s -o /dev/null -X POST "$B/v1/messages" \
  ...
  -d '{"model":"claude-3-haiku-20240307","max_tokens":5,"metadata":{"user_id":"slot3-max-headers"},"messages":[{"role":"user","content":"hi"}]}'
...
curl -s -o /dev/null -X POST "$B/v1/complete" \
  -H "content-type: application/json" \
  -d '{"model":"claude-3-opus-latest","prompt":"x","metadata":{"user_id":"other-path"}}'
...
curl -s -o "$OUT/passthrough.body" -w "%{http_code}" -X POST "$B/v1/messages" \
  -H "content-type: application/json" \
  -d '{"model":"claude-3-sonnet-20240229","max_tokens":5,"output_config":{"effort":"low"},"thinking":{"type":"adaptive"},"metadata":{"user_id":"slot2-passthrough"},"messages":[{"role":"user","content":"hi"}]}' > "$OUT/passthrough.status"
...
curl -s -o "$OUT/rectify.body" -w "%{http_code}" -X POST "$B/v1/messages" \
  -H "content-type: application/json" \
  -d '{"model":"claude-3-opus-latest","max_tokens":5,"output_config":{"effort":"max"},"metadata":{"user_id":"effort-reject-1"},"messages":[{"role":"user","content":"hi"}]}' > "$OUT/rectify.status"

# 13) 同一服务商+模型的下一发：能力缓存命中，effort 直接不发（只有一次往返）
curl -s -o /dev/null -X POST "$B/v1/messages" \
  -H "content-type: application/json" \
  -d '{"model":"claude-3-opus-latest","max_tokens":5,"output_config":{"effort":"max"},"metadata":{"user_id":"effort-reject-2"},"messages":[{"role":"user","content":"hi"}]}'
